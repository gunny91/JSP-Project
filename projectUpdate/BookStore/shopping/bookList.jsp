...
if(managerId == null || managerId.equals("")) {
	...
	pw.println("location.href='logon/loginPro.jsp?useSSl=false'");
	pw.println("</script>");
}
			...
			<td>
				<div class="btn-group">
					...
					<select class="form-control" name="book_kind" onchange="location.href=this.value">
						<option value="bookList.jsp?book_kind_all">전체목록보기</option>
						<%for(int i =0; i<bookTypes.size();i++){
							bookType=(BscodeDTO)bookTypes.get(i);%>
		...
		<tr>
			...
			<td align=center>
				<a href="bookContent.jsp?book_id?=<%=
					book.getBook_id()%>&book_kind<%=URLEncoder.encode(book.getBook_kind(),"UTF-8") %> ">
				</a>
			</td>
			...
			<td align=right style="color:red">
				<!--  -->
				<%=book.getBook_price()*(100-book.getDiscount_rate())/100 %>
			</td>
